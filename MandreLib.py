...
# --- ИМПОРТЫ ДЛЯ SQL (Добавить вверх файла если нет) ---
try:
    SQLiteDatabase = find_class("org.telegram.SQLite.SQLiteDatabase")
    SQLiteCursor = find_class("org.telegram.SQLite.SQLiteCursor")
    SQLitePreparedStatement = find_class("org.telegram.SQLite.SQLitePreparedStatement")
except:
    pass
...
class _MandreDB:
    ...
    def __init__(self):
        self.db_path = os.path.join(str(ApplicationLoader.getFilesDirFixed()), "mandre_storage_v2.db")
        self.db = None
        self._init_db()
    ...
    def _init_db(self):
        try:
            ...
            # MEGA PERFORMANCE TUNING
            self.db.executeFast("PRAGMA journal_mode = WAL").stepThis().dispose()
            self.db.executeFast("PRAGMA synchronous = OFF").stepThis().dispose()
            self.db.executeFast("PRAGMA temp_store = MEMORY").stepThis().dispose()
            
            # Таблица KV: plugin_id (индекс), key (имя файла/ключ), value (данные)
            self.db.executeFast(
            ...
            self.db.executeFast(
                "CREATE INDEX IF NOT EXISTS idx_plugin ON kv_storage (plugin_id)"
            ).stepThis().dispose()
            
            log("[MandreData] SQL Storage V2 initialized (WAL Mode)")
        except Exception as e:
            log(f"[MandreData] DB Init Critical Error: {e}")

    @staticmethod
    def get():
        with _MandreDB._lock:
            if not _MandreDB._instance:
                _MandreDB._instance = _MandreDB()
            return _MandreDB._instance

    def upsert(self, plugin_id, key, data_str):
        with _MandreDB._lock:
            stmt = self.db.executeFast("REPLACE INTO kv_storage VALUES (?, ?, ?)")
            stmt.bindString(1, str(plugin_id))
            stmt.bindString(2, str(key))
            stmt.bindString(3, str(data_str))
            stmt.stepThis().dispose()

    def select(self, plugin_id, key):
        with _MandreDB._lock:
            cursor = self.db.queryFinalized(f"SELECT value FROM kv_storage WHERE plugin_id = '{plugin_id}' AND key = '{key}' LIMIT 1")
            try:
                if cursor.next():
    ...
    def delete(self, plugin_id, key):
        with _MandreDB._lock:
            self.db.executeFast(f"DELETE FROM kv_storage WHERE plugin_id = '{plugin_id}' AND key = '{key}'").stepThis().dispose()

    def delete_plugin(self, plugin_id):
        with _MandreDB._lock:
            self.db.executeFast(f"DELETE FROM kv_storage WHERE plugin_id = '{plugin_id}'").stepThis().dispose()

    def list_keys(self, plugin_id):
        keys = []
        with _MandreDB._lock:
            cursor = self.db.queryFinalized(f"SELECT key FROM kv_storage WHERE plugin_id = '{plugin_id}'")
            try:
                while cursor.next():
                    keys.append(cursor.stringValue(0))
            finally:
                cursor.dispose()
        return keys
    ...
    def list_plugins(self):
        plugins = []
        with _MandreDB._lock:
            cursor = self.db.queryFinalized("SELECT DISTINCT plugin_id FROM kv_storage")
            try:
                while cursor.next():
                    plugins.append(cursor.stringValue(0))
            finally:
                cursor.dispose()
        return plugins
...
class MandreData:
    """
    Переписанный модуль данных.
    Использует SQLite, MMM, MMC и MandreAsync.
    Поддерживает прозрачную миграцию со старых JSON файлов.
    """

    # --- PRIVATE HELPERS ---

    @staticmethod
    def _migrate_check(plugin_id: str, filename: str):
        """
        Проверяет наличие старого файла. Если есть - читает, пишет в SQL и удаляет файл.
        """
        # Старый путь
        base_dir = File(ApplicationLoader.getFilesDirFixed(), "plugins")
        data_dir = File(base_dir, "mandre_lib_data")
        plugin_dir = File(data_dir, plugin_id)
        legacy_file = File(plugin_dir, filename)

        if legacy_file.exists():
            try:
                log(f"[MandreData] Migrating {plugin_id}/{filename} to SQL...")
                with open(legacy_file.getAbsolutePath(), 'r', encoding='utf-8') as f:
                    raw_data = f.read()
                
                # Пишем в SQL как есть (строкой)
                _MandreDB.get().upsert(plugin_id, filename, raw_data)
                
                # Удаляем файл
                legacy_file.delete()
                
                # Удаляем папку, если пустая
                if len(plugin_dir.list() or []) == 0:
                    plugin_dir.delete()
                    
                return raw_data
            except Exception as e:
                log(f"[MandreData] Migration failed: {e}")
        return None

    @staticmethod
    def _serialize(data):
        # Используем MMC для тяжелой сериализации, если данных много
        # Но для JSON стандартный json.dumps достаточно быстр.
    ...
    @staticmethod
    def set(plugin_instance, key: str, value: Any):
        """Супер-быстрая запись Key-Value"""
        pid = plugin_instance.id if hasattr(plugin_instance, 'id') else str(plugin_instance)
        serialized = MandreData._serialize(value)
        _MandreDB.get().upsert(pid, key, serialized)

    @staticmethod
    def get(plugin_instance, key: str, default: Any = None) -> Any:
        """Супер-быстрое чтение Key-Value"""
        pid = plugin_instance.id if hasattr(plugin_instance, 'id') else str(plugin_instance)
        val = _MandreDB.get().select(pid, key)
        if val is not None:
            return MandreData._deserialize(val)
        return default

    # --- COMPATIBILITY API (JSON MAPPED TO SQL) ---

    @staticmethod
    def write_persistent_json(plugin_id: str, filename: str, data: Any):
        """
        Legacy: Синхронная запись.
        Мапит 'filename' в ключ таблицы.
        """
        try:
            # MMC OFFLOAD: Если данных много, сериализуем в треде
            # Но так как метод синхронный, делаем тут.
            serialized = MandreData._serialize(data)
            _MandreDB.get().upsert(plugin_id, filename, serialized)
        except Exception as e:
            log(f"[MandreData] Write Error: {e}")

    @staticmethod
    def read_persistent_json(plugin_id: str, filename: str, default: Any = None) -> Any:
        """
        Legacy: Синхронное чтение с авто-миграцией.
        """
        try:
            # 1. Попытка чтения из SQL (Fast RAM/Cache)
            val = _MandreDB.get().select(plugin_id, filename)
            
            # 2. Если в базе нет, проверяем диск (Migration)
            if val is None:
                val = MandreData._migrate_check(plugin_id, filename)
            
            if val is not None:
                # Используем MMM (Memory) если строка огромная (для оптимизации, условно)
                if len(val) > 500_000: # 500kb+
                    # Пример использования MMM (просто аллокация для теста)
                    # В реальности JSON парсер сам все сделает
                    pass 
                return MandreData._deserialize(val)
                
            return default
        except Exception as e:
            log(f"[MandreData] Read Error: {e}")
            return default

    # --- ASYNC API (POWERED BY MandreAsync) ---

    @staticmethod
    async def write_persistent_json_async(plugin_id: str, filename: str, data: Any):
        """
        Асинхронная запись. Использует MMC для сериализации и MandreAsync для IO.
        """
        if not hasattr(Mandre, "MultiCore") or not hasattr(Mandre, "Async"):
            # Fallback
            return MandreData.write_persistent_json(plugin_id, filename, data)

        try:
            # 1. Тяжелая сериализация на другом ядре (bypass GIL constraint partially)
            serialized = await Mandre.MultiCore.run_async(json.dumps, data)
            
            # 2. Запись в БД (SQLite thread-safe, но можно вынести в тред)
            # Так как executeFast быстрый, можно вызвать напрямую, но для чистоты:
            def db_write():
                _MandreDB.get().upsert(plugin_id, filename, serialized)
            
            await Mandre.Async.run(db_write)
    ...
    async def read_persistent_json_async(plugin_id: str, filename: str, default: Any = None) -> Any:
        ...
        try:
            # 1. Чтение из БД в отдельном потоке
            def db_read():
                val = _MandreDB.get().select(plugin_id, filename)
                if val is None:
                    val = MandreData._migrate_check(plugin_id, filename)
            ...
            val = await Mandre.Async.run(db_read)
            
            if val:
                # 2. Десериализация на другом ядре
                return await Mandre.MultiCore.run_async(json.loads, val)
    ...
    # --- FILE MANAGEMENT UTILS (MAPPED TO SQL) ---

    @staticmethod
    def list_persistent_plugins() -> List[str]:
        return _MandreDB.get().list_plugins()
    
    @staticmethod
    def list_files_for_plugin(plugin_id: str) -> List[str]:
        # Возвращаем ключи как имена файлов для совместимости
        return _MandreDB.get().list_keys(plugin_id)

    @staticmethod
    def delete_persistent_plugin_data(plugin_id: str) -> bool:
        try:
            _MandreDB.get().delete_plugin(plugin_id)
            
            # Также подчищаем физическую папку на всякий случай
            base_dir = File(ApplicationLoader.getFilesDirFixed(), "plugins")
            data_dir = File(base_dir, "mandre_lib_data")
            plugin_dir = File(data_dir, plugin_id)
            if plugin_dir.exists():
                shutil.rmtree(plugin_dir.getAbsolutePath())
                
            log(f"[MandreData] Wiped data for {plugin_id}")
            return True
        except Exception as e:
            log(f"[MandreData] Delete Error: {e}")
            return False

    @staticmethod
    def get_persistent_path(plugin_id: str, filename: str) -> str:
        """
        DEPRECATED: SQL не имеет путей.
        Но для совместимости возвращаем виртуальный путь или создаем временный файл.
        """
        # Если плагин требует реальный файл (например для отправки), выгружаем его из БД во временный
        val = _MandreDB.get().select(plugin_id, filename)
        
        # Создаем структуру папок для совместимости
        base_dir = File(ApplicationLoader.getFilesDirFixed(), "plugins")
        data_dir = File(base_dir, "mandre_lib_data")
        plugin_dir = File(data_dir, plugin_id)
        if not plugin_dir.exists(): plugin_dir.mkdirs()
        
        f = File(plugin_dir, filename)
        
        # Если данные есть в базе, дампим их в файл, чтобы старый код мог прочитать путь
        if val:
            with open(f.getAbsolutePath(), 'w', encoding='utf-8') as fs:
                fs.write(val)
...
# --- ВНУТРЕННЯЯ ЛОГИКА (CLEAN ARCHITECTURE) ---

class _DeviceInternals:
    """
    Внутренний класс для работы с железом. 
    Изолирован от публичного API для безопасности и чистоты.
    """
    _CACHE = {}

    @staticmethod
    def get_static_identifiers():
        """Возвращает неизменяемые данные устройства (с кэшированием)."""
        if _DeviceInternals._CACHE:
            return _DeviceInternals._CACHE.copy()

        info = {
            # Hardware
            "manufacturer": str(Build.MANUFACTURER).title(),
            "model": str(Build.MODEL),
            "brand": str(Build.BRAND).title(),
            ...
            "codename": str(Build.VERSION.CODENAME),
            "incremental": str(Build.VERSION.INCREMENTAL),
            ...
            "build_host": str(Build.HOST),
            
            # ABI / CPU
            "cpu_abi": str(Build.CPU_ABI),
            "cpu_abi2": str(Build.CPU_ABI2),
            "supported_abis": list(Build.SUPPORTED_ABIS) if hasattr(Build, 'SUPPORTED_ABIS') else [],
            
            # Features
            "is_emulator": _DeviceInternals._check_emulator(),
            "is_rooted": _DeviceInternals._check_root(),
        }
        
        _DeviceInternals._CACHE = info
        return info

    @staticmethod
    def get_dynamic_display(context):
        """Получает текущие метрики экрана."""
        try:
    ...
    @staticmethod
    def get_dynamic_memory(context):
        """Получает текущее состояние RAM."""
        try:
            am = context.getSystemService(Context.ACTIVITY_SERVICE)
            mi = ActivityManager.MemoryInfo()
            am.getMemoryInfo(mi)
            return {
                "total_memory_mb": int(mi.totalMem / 1048576),
                "available_memory_mb": int(mi.availMem / 1048576),
                "low_memory_state": bool(mi.lowMemory)
            }
        except: pass
        return {}

    @staticmethod
    def get_safe_telephony(context):
        """
        Безопасное получение данных сети. 
        SENSITIVE DATA STRIPPED (Privacy by Design).
        """
        data = {
            "phone_type": 0,
            "network_operator": "Unknown",
            "network_operator_name": "Unknown",
            "sim_operator": "Unknown",
            "sim_operator_name": "Unknown",
            "sim_country_iso": "Unknown",
            # PRIVACY STUBS (Для совместимости ключи оставлены, данные скрыты)
            "sim_serial": "[PROTECTED_BY_MANDRE]",
            "subscriber_id": "[PROTECTED_BY_MANDRE]",
            "device_id": "[PROTECTED_BY_MANDRE]",
            "imei": "[PROTECTED_BY_MANDRE]",
            "line1_number": "[PROTECTED_BY_MANDRE]"
        }
        
        try:
            tm = context.getSystemService(Context.TELEPHONY_SERVICE)
            if tm:
                data["phone_type"] = tm.getPhoneType()
                data["network_operator"] = str(tm.getNetworkOperator() or "Unknown")
                data["network_operator_name"] = str(tm.getNetworkOperatorName() or "Unknown")
                data["sim_operator"] = str(tm.getSimOperator() or "Unknown")
                data["sim_operator_name"] = str(tm.getSimOperatorName() or "Unknown")
                data["sim_country_iso"] = str(tm.getSimCountryIso() or "Unknown")
        except: pass
        return data
    ...
    def get_app_info(context):
        try:
            p_info = context.getPackageManager().getPackageInfo(context.getPackageName(), 0)
            return {
                "app_version_name": str(p_info.versionName),
                "app_version_code": int(p_info.versionCode),
                "app_package": str(context.getPackageName())
            }
        except:
            return {"app_error": "Package info failed"}

    @staticmethod
    def _check_emulator():
        """Детект эмулятора."""
        fp = str(Build.FINGERPRINT)
        model = str(Build.MODEL)
        man = str(Build.MANUFACTURER)
        product = str(Build.PRODUCT)
        hardware = str(Build.HARDWARE)
        return (
            "generic" in fp or "google_sdk" in model or 
            "Genymotion" in man or "goldfish" in hardware or 
            "sdk" in product
        )

    @staticmethod
    def _check_root():
        """Детект рута по наличию бинарников."""
        paths = [
...
class MandreDevice:
    """
    Модуль информации об устройстве.
    Optimized & Privacy-Safe.
    """

    @staticmethod
    def get_device_info() -> Dict[str, Any]:
        """
        Собирает полную информацию об устройстве.
        Безопасно для использования в логах (без IMEI/IMSI).
        """
        ctx = ApplicationLoader.applicationContext
        if not ctx:
            return {"error": "Context unavailable"}

        # 1. Берем статику из кэша (мгновенно)
        info = _DeviceInternals.get_static_identifiers()

        # 2. Обогащаем динамическими данными
        info.update(_DeviceInternals.get_dynamic_display(ctx))
        info.update(_DeviceInternals.get_dynamic_memory(ctx))
        info.update(_DeviceInternals.get_safe_telephony(ctx))
        info.update(_DeviceInternals.get_app_info(ctx))

        # 3. Метаданные
        info["timestamp"] = int(time.time())
        info["locale"] = str(Locale.getDefault())
        info["timezone"] = str(TimeZone.getDefault().getID())
    ...
    def get_simple_info() -> str:
        """
        Возвращает краткую строку: 'Model (Android X)'.
        """
        model = str(Build.MODEL or "Unknown")
        ver = str(Build.VERSION.RELEASE or "?")
        api = str(Build.VERSION.SDK_INT)
        return f"{model} (Android {ver}, API {api})"

    @staticmethod
    def is_tablet() -> bool:
        """Определяет, планшет это или телефон (по размеру экрана)."""
        try:
            ctx = ApplicationLoader.applicationContext
            ...
            sl = res.getConfiguration().screenLayout & 15
            return sl >= 3
        except:
            return False
            
    # --- DEPRECATED ALIASES (Compatibility Layer) ---
    
    @staticmethod
    def _is_emulator(): 
        # Для старых плагинов, вызывающих приватный метод напрямую
        return _DeviceInternals._check_emulator()
        
    @staticmethod
    def _is_rooted():
        # Для старых плагинов
        return _DeviceInternals._check_root()

# !!! ВСТАВЬТЕ ССЫЛКУ НА mandre_v8.dex !!!
DEX_URL = "https://github.com/coidarashka/MandreLib-Modules/raw/refs/heads/main/ya%20ne%20navishu%20dex%20blya%20vodka%20ak47%20settings%20by%20swaga%20huyaga%20v%20pizdy.dex"
DEX_CLASS_NAME = "dex.mandre.MandreSettingsDex"

class MandreDex:
    _loader_class = None
    _parse_method = None
    _update_method = None  # NEW
    _is_loaded = False
    _is_loading = False
    ...
    def ensure_loaded(plugin_instance):
        ...
        def load_task():
            try:
                ...
                # v10 для сброса кэша (изменил версию, чтобы перезакачался файл)
                dex_path = os.path.join(str(context.getFilesDir()), "mandre_v10 misha strani chel non swag.dex")

                if not os.path.exists(dex_path):
                    plugin_instance.log(f"[MandreDex] Downloading v10 from {DEX_URL}...")
                    try:
                ...
                try:
                    ...
                    # СОХРАНЯЕМ КЛАСС ЗДЕСЬ, ЧТОБЫ ИСПОЛЬЗОВАТЬ ПОЗЖЕ
                    ...
                    # Метод обновления UI
                    MandreDex._update_method = MandreDex._loader_class.getDeclaredMethod(
                        "updateView", StringCls, StringCls, StringCls
                    )
                    
                    MandreDex._is_loaded = True
                    plugin_instance.log("[MandreDex] Engine v10 Loaded.")
            ...
            def handleMessage(self, msg):
                try:
                    ...
                    if what == 1: # LOG
                        ...
                        elif method.startswith("__time__:"):
                            # Handle Time Picker internally
                            parts = method.split(":")
                            key = parts[1]
                            curr = parts[2].split(":")
                            h, m = int(curr[0]), int(curr[1])
                            
                            def on_time(nh, nm):
                                val = f"{nh:02}:{nm:02}"
                                plugin_instance.set_setting(key, val)
                                MandreDex.update_view(key, "text", f"🕒 {val}")
                                
                            MandreUI.pick_time(on_time, h, m)
    ...
    @staticmethod
    def update_view(key, prop, value):
        """Безопасное обновление View через загруженный класс"""
        if not MandreDex._is_loaded or not MandreDex._update_method: return
        try:
            run_on_ui_thread(lambda: MandreDex._update_method.invoke(None, str(key), str(prop), str(value)))
        except Exception as e:
            log(f"[MandreDex] Update Error: {e}")
...
# --- ИМПОРТЫ (Добавить вверх файла, если нет) ---
try:
    StrictMode = find_class("android.os.StrictMode")
except:
    pass

# --- ВНУТРЕННИЙ БЭКЕНД (Изоляция логики) ---

class _ShareBackend:
    """
    Приватный класс для низкоуровневой работы с Intent и файловой системой.
    """
    
    # Статический кэш MIME-типов для мгновенного доступа
    _MIME_CACHE = {
        'jpg': 'image/jpeg', 'jpeg': 'image/jpeg', 'png': 'image/png', 'gif': 'image/gif', 'webp': 'image/webp',
        'mp4': 'video/mp4', 'webm': 'video/webm', 'mov': 'video/quicktime', 'mkv': 'video/x-matroska',
        'mp3': 'audio/mpeg', 'wav': 'audio/wav', 'ogg': 'audio/ogg', 'm4a': 'audio/mp4', 'flac': 'audio/flac',
        'pdf': 'application/pdf', 'doc': 'application/msword', 'txt': 'text/plain', 'zip': 'application/zip',
        'json': 'application/json', 'xml': 'application/xml', 'html': 'text/html', 'apk': 'application/vnd.android.package-archive'
    }

    @staticmethod
    def get_mime(path: str) -> str:
        """Определяет MIME по расширению за O(1)."""
        try:
            ext = path.rsplit('.', 1)[-1].lower() if '.' in path else ''
            return _ShareBackend._MIME_CACHE.get(ext, 'application/octet-stream')
        except:
            return 'application/octet-stream'

    @staticmethod
    def prepare_file_async(src_path: str, context: Context):
        """
        Копирует файл в публичную директорию для шаринга.
        Выполняется в фоновом потоке.
        """
        try:
            # 1. Проверки
            if not os.path.exists(src_path): raise FileNotFoundError("File not found")
            
            src_file = File(src_path)
            filename = src_file.getName()
            
            # 2. Целевая папка (Cache или External для совместимости)
            # Используем кэш приложения, так как FileProvider настроен на него безопаснее
            cache_dir = File(context.getExternalCacheDir(), "shared_files")
            if not cache_dir.exists(): cache_dir.mkdirs()
            
            dest_file = File(cache_dir, filename)
            
            # 3. Копирование (Самая тяжелая часть)
            # Используем shutil, он оптимизирован на уровне C
            shutil.copy2(src_path, dest_file.getAbsolutePath())
            
            # 4. Формирование URI
            authority = ApplicationLoader.getApplicationId() + ".provider"
            
            # Хак для FileProvider, чтобы избежать крашей на некоторых Android
            try:
                vm_policy = StrictMode.getVmPolicy()
                StrictMode.setVmPolicy(StrictMode.VmPolicy.Builder().build())
            except: pass
            
            uri = FileProvider.getUriForFile(context, authority, dest_file)
            
            # Возвращаем путь для очистки и URI для интента
            return dest_file.getAbsolutePath(), uri
            
        except Exception as e:
            log(f"[MandreShare] File Prep Error: {e}")
            return None, None

    @staticmethod
    def schedule_cleanup(path: str, delay_sec: int = 300):
        ...
        def cleaner():
            try:
                ...
                if os.path.exists(path):
                    os.remove(path)
                    log(f"[MandreShare] Cleaned up: {os.path.basename(path)}")
            except: pass
        
        # Запускаем в отдельном демоне, чтобы не висел в пуле MandreAsync
...
class MandreShare:
    """
    Модуль для шеринга контента в другие приложения.
    Async-Powered, Non-Blocking UI.
    """
    ...
    def share_text(text: str, title: str = "Поделиться"):
        ...
        """
        if not text: return BulletinHelper.show_error("Нет текста для отправки")
        ...
        def ui_task():
            try:
                ...
                intent.putExtra(Intent.EXTRA_TEXT, str(text))
                intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)

                chooser = Intent.createChooser(intent, str(title))
                chooser.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
                ctx.startActivity(chooser)
                
            except Exception as e:
                log(f"[MandreShare] Text Error: {e}")
                BulletinHelper.show_error("Ошибка UI при отправке")
    ...
    def share_file(file_path: str, title: str = "Поделиться файлом", mime_type: str = None):
        """
        Асинхронно подготавливает файл и открывает диалог шаринга.
        Не блокирует UI даже на больших файлах.
        """
        if not file_path: return BulletinHelper.show_error("Путь не указан")
        
        # Показываем лоадер, если файл большой, иначе юзер подумает что кнопка не нажалась
        is_large = False
        try:
            if os.path.getsize(file_path) > 5 * 1024 * 1024: # > 5MB
                is_large = True
                BulletinHelper.show_info("Подготовка файла...")
        except: pass
        ...
        def background_task():
            try:
                ...
                # 1. Копирование и генерация URI (Блокирующая операция)
                dest_path, uri = _ShareBackend.prepare_file_async(file_path, ctx)
                
                if not dest_path or not uri:
                    run_on_ui_thread(lambda: BulletinHelper.show_error("Ошибка доступа к файлу"))
                    return

                # 2. Определение MIME
                final_mime = mime_type or _ShareBackend.get_mime(file_path)
                ...
                def ui_launch():
                    try:
                        ...
                        if not act: 
                            act = ctx # Fallback to App Context if Activity dead
                        
                        intent = Intent(Intent.ACTION_SEND)
                        intent.setType(final_mime)
                        intent.putExtra(Intent.EXTRA_STREAM, uri)
                        intent.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION)
                        
                        # Fix для Android 10+
                        intent.setDataAndType(uri, final_mime)
                        
                        chooser = Intent.createChooser(intent, str(title))
                        if not frag: chooser.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
                        
                        act.startActivity(chooser)
                        if is_large: BulletinHelper.show_success("Готово к отправке")
                        
                        # Планируем очистку копии
                        _ShareBackend.schedule_cleanup(dest_path)
                        
                    except Exception as e:
                        log(f"[MandreShare] UI Error: {e}")
                        BulletinHelper.show_error(f"Ошибка Intent: {e}")

                run_on_ui_thread(ui_launch)

            except Exception as e:
                log(f"[MandreShare] Critical: {e}")

        # Используем MandreAsync если доступен, иначе тред
        if hasattr(Mandre, "Async"):
            Mandre.Async.run(background_task)
        else:
...
class MandreUI:
    ...
    @staticmethod
    def open_internal_link(url_or_username: str):
        """
        Открывает ссылку или юзернейм внутри Telegram (как клик по тегу).
        Пример: Mandre.UI.open_internal_link("@mandrelib")
        """
        def runner():
            try:
                fragment = get_last_fragment()
                ...
                # Используем встроенный браузер Telegram, он сам решит, 
                # открывать это как чат, профиль или веб-страницу
                Browser = find_class("org.telegram.messenger.browser.Browser")
                Browser.openUrl(activity, url)
                
            except Exception as e:
                log(f"[MandreUI] Open Link Error: {e}")
...
class MandreMessages:
    ...
    @staticmethod
    def edit(plugin_instance, message_object, new_text: str, buttons: List[List[Dict]] = None):
        """
        Умное редактирование сообщения.
        :param plugin_instance: self (из плагина)
        :param message_object: MessageObject
        :param new_text: Текст с Markdown (**bold**, и т.д.)
        :param buttons: Список кнопок (как в create_markup). Если None - кнопки не меняются (или удаляются, зависит от логики).
        """
        try:
            # 1. Парсинг текста
            parsed = Mandre.Text.parse(new_text)
            ...
            if buttons is not None:
                hidden_payload = Mandre.Inline.create_markup(plugin_instance, buttons)
            
            # Если кнопок не передали, но в старом тексте был маркер, 
            # по хорошему надо бы его сохранить, но мы предполагаем полную замену контента.
            
            final_text = clean_text + hidden_payload

            # 3. Подготовка запроса
            # Безопасный поиск классов
            try:
                TLRPC = find_class("org.telegram.tgnet.TLRPC")
                EditMsg = TLRPC.TL_messages_editMessage
            except:
                return

            req = EditMsg()
            req.id = message_object.getId()
            req.message = final_text
            
            # Флаги: 2048 (no_webpage)
            req.flags = 2048
            ...
            # 4. Поиск Peer (InputPeer)
            mc = get_messages_controller()
            did = message_object.getDialogId()
            input_peer = mc.getInputPeer(did)
            
            # Фолбэк, если peer не найден (например в сохраненках иногда)
            if not input_peer and message_object.messageOwner and message_object.messageOwner.peer_id:
                 # Пытаемся угадать
                 try:
                     uid = message_object.messageOwner.peer_id.user_id
            ...
            if not input_peer:
                BulletinHelper.show_error("Edit Error: Peer not found")
                return

            req.peer = input_peer

            # 5. Отправка и обновление UI
            def cb(resp, err):
                if resp:
                    # Принудительно обновляем UI
                    try: mc.processUpdates(resp, False)
                    except: pass
                
                if err:
                    txt = str(err.text)
                    if "NOT_MODIFIED" not in txt:
                         run_on_ui_thread(lambda: BulletinHelper.show_error(f"Edit Error: {txt}"))

            # Отправляем через client_utils (уже импортирован в MandreLib)
            from client_utils import send_request, RequestCallback
            send_request(req, RequestCallback(cb))

        except Exception as e:
            log(f"[MandreMessages] Edit Exception: {traceback.format_exc()}")
...
# --- LUA ENGINE "ULTIMATE BRIDGE" (FINAL) ---
try:
    import ctypes
    import threading
    import json
    ...
    from ctypes import c_void_p, c_char_p, c_int, c_long, c_double, POINTER, CFUNCTYPE, cast
except ImportError:
    ctypes = None

# Глобальное хранилище для хуков, чтобы GC не убил прокси-объекты
_LUA_HOOKS_STORE = []

# --- LUA BOOT SCRIPT ---
# Это "ядро" на Lua, которое инициализируется при старте.
# Оно добавляет глобальную таблицу 'java' и систему проксирования.
LUA_BOOT_SCRIPT = r"""
-- [[ TINY JSON LIB ]] --
local json={};local function kind_of(a)if type(a)~='table'then return type(a)end;local b=1;for c in pairs(a)do if a[b]~=nil then b=b+1 else return'table'end end;if b==1 then return'table'else return'array'end end;local function escape_str(a)local d={['"']='\\"',['\\']='\\\\',['/']='\\/',['\b']='\\b',['\f']='\\f',['\n']='\\n',['\r']='\\r',['\t']='\\t'}return'"'..string.gsub(a,'[%z\1-\31\\"]',d)..'"'end;function json.stringify(a,e)e=e or{}if type(a)=='table'then if e[a]then error('cycle')end;e[a]=true;local f,g={},{}local h=kind_of(a)if h=='array'then for b=1,#a do table.insert(f,json.stringify(a[b],e))end;e[a]=nil;return'['..table.concat(f,',')..']'elseif h=='table'then for c,i in pairs(a)do if type(c)=='string'then table.insert(f,escape_str(c)..':'..json.stringify(i,e))end end;e[a]=nil;return'{'..table.concat(f,',')..'}'end else if type(a)=='string'then return escape_str(a)elseif type(a)=='number'then return tostring(a)elseif type(a)=='boolean'then return tostring(a)elseif type(a)=='nil'then return'null'else error('json type error: '..type(a))end end end

-- [[ JAVA BRIDGE SYSTEM ]] --
java = {}
local meta_java = {}

-- Создает Lua-обертку для Java-объекта по ID
function java.wrap(id)
    local obj = { __id = id, __type = "java_obj" }
    setmetatable(obj, meta_java)
    return obj
end

-- Импорт класса (эквивалент import в Python/Java)
function java.import(name)
    local id = _py_bridge_import(name)
    if id == -1 then error("Java class not found: " .. name) end
    return java.wrap(id)
end

-- Магия: вызов методов (obj.method(...) или obj:method(...))
function meta_java:__index(key)
    return function(self, ...)
        local args = {...}
        local clean_args = {}
        
        -- Подготовка аргументов: заменяем объекты на их ID
        for i, v in ipairs(args) do
            if type(v) == 'table' and v.__type == 'java_obj' then
                table.insert(clean_args, {__java_ref = v.__id})
            else
                table.insert(clean_args, v)
            end
        end
        
        -- Вызов Python-моста через JSON
        local args_json = json.stringify(clean_args)
        local res_raw = _py_bridge_call(self.__id, key, args_json)
        
        -- Парсинг ответа (протокол: "T<content>")
        local type_marker = string.sub(res_raw, 1, 1)
        local content = string.sub(res_raw, 2)
        
        if type_marker == "E" then error(content) -- Error
        elseif type_marker == "R" then return java.wrap(tonumber(content)) -- Reference
        elseif type_marker == "N" then return nil
        elseif type_marker == "S" then return content -- String
        elseif type_marker == "B" then return content == "true" -- Bool
        elseif type_marker == "I" then return tonumber(content) -- Int/Float
        else return content end
    end
end

-- Вызов конструктора или самого объекта
function meta_java:__call(...)
    return self:__init(...)
end

-- Сборщик мусора: когда Lua удаляет объект, говорим Python забыть его
function meta_java:__gc()
    _py_bridge_release(self.__id)
end
"""

class MandreLua:
    _lib = None
    LIB_URL = "https://github.com/coidarashka/MandreCompileFilles/raw/refs/heads/main/liblua.so"
    LUA_C_FUNC = None 
    # Константа реестра для Lua 5.4
    LUA_REGISTRYINDEX = -1001000 

    @staticmethod
    def ensure_lib(plugin_instance):
        if not ctypes: return False
        if MandreLua._lib: return True
        
        files_dir = str(ApplicationLoader.getFilesDirFixed())
        lib_path = os.path.join(files_dir, "liblua.so")
        
        # Авто-скачивание
        if not os.path.exists(lib_path):
            plugin_instance.log(f"[MandreLua] Downloading engine...")
            try:
                r = requests.get(MandreLua.LIB_URL, timeout=30)
                with open(lib_path, "wb") as f: f.write(r.content)
            except Exception as e:
                plugin_instance.log(f"[MandreLua] Download Error: {e}")
                return False
            
        try:
            MandreLua._lib = ctypes.CDLL(lib_path)
            l = MandreLua._lib
            
            # --- Настройка сигнатур функций (ArgTypes) ---
            # Это критически важно для ARM64
            l.luaL_newstate.restype = c_void_p
            l.luaL_openlibs.argtypes = [c_void_p]
            l.luaL_loadstring.argtypes = [c_void_p, c_char_p]; l.luaL_loadstring.restype = c_int
            l.lua_pcallk.argtypes = [c_void_p, c_int, c_int, c_int, c_int, c_void_p]; l.lua_pcallk.restype = c_int
            l.lua_tolstring.argtypes = [c_void_p, c_int, c_void_p]; l.lua_tolstring.restype = c_char_p
            l.lua_pushstring.argtypes = [c_void_p, c_char_p]
            l.lua_pushinteger.argtypes = [c_void_p, c_long]
            l.lua_tointegerx.argtypes = [c_void_p, c_int, c_void_p]; l.lua_tointegerx.restype = c_long
            l.lua_setglobal.argtypes = [c_void_p, c_char_p]
            l.lua_getglobal.argtypes = [c_void_p, c_char_p]; l.lua_getglobal.restype = c_int
            l.lua_pushcclosure.argtypes = [c_void_p, c_void_p, c_int]
            l.lua_settop.argtypes = [c_void_p, c_int]
            l.lua_close.argtypes = [c_void_p]
            
            MandreLua.LUA_C_FUNC = CFUNCTYPE(c_int, c_void_p)
            return True
        except Exception as e:
            plugin_instance.log(f"[MandreLua] Init Error: {traceback.format_exc()}")
            return False

    def __init__(self, plugin_instance):
        self.plugin = plugin_instance
        self.L = None
        self._callbacks = [] # Храним ссылки на C-функции
        self.lock = threading.RLock() # Защита от гонки потоков
        
        # --- JAVA OBJECT REGISTRY ---
        # Здесь мы храним реальные Java-объекты, пока Lua работает с их ID
        self.registry = {} 
        self.next_id = 1
        
        if MandreLua.ensure_lib(plugin_instance):
            try:
                self.L = MandreLua._lib.luaL_newstate()
                if self.L:
                    MandreLua._lib.luaL_openlibs(self.L)
                    self._register_api()
                    # Инициализируем Lua-сторону моста
                    err = self.run_script(LUA_BOOT_SCRIPT)
                    if err: self.plugin.log(f"[MandreLua] Boot Error: {err}")
            except Exception as e:
                self.plugin.log(f"[MandreLua] State Error: {e}")

    # --- BRIDGE HELPERS ---

    def _reg_obj(self, obj):
        """Сохраняет объект в реестр и возвращает ID"""
        if obj is None: return -1
        idx = self.next_id
        self.next_id += 1
        self.registry[idx] = obj
        return idx

    def _get_obj(self, idx):
        return self.registry.get(idx)

    def _free_obj(self, idx):
    ...
    def _register_api(self):
        # 1. log/print
        def lua_log(L):
            msg = self._get_str(L, 1)
            self.plugin.log(f"[Lua] {msg}")
            return 0
        ...
        # 3. Settings: get_setting
        def lua_get_set(L):
            k = self._get_str(L, 1)
            d = self._get_str(L, 2)
            v = self.plugin.get_setting(k, d)
            MandreLua._lib.lua_pushstring(L, str(v).encode('utf-8'))
            return 1

        # 4. Settings: set_setting
        def lua_set_set(L):
            k = self._get_str(L, 1)
            v = self._get_str(L, 2)
            self.plugin.set_setting(k, v)
            return 0

        # 5. BRIDGE: Import Class -> ID
        def py_import(L):
            cls_name = self._get_str(L, 1)
            try:
                # Используем мощь Chaquopy для импорта
                cls = jclass(cls_name)
                idx = self._reg_obj(cls)
                MandreLua._lib.lua_pushinteger(L, idx)
            except Exception as e:
                self.plugin.log(f"Import Err: {cls_name} -> {e}")
                MandreLua._lib.lua_pushinteger(L, -1)
            return 1

        # 6. BRIDGE: Call Method -> Encoded Result
        def py_call(L):
            try:
                obj_id = MandreLua._lib.lua_tointegerx(L, 1, None)
                method_name = self._get_str(L, 2)
                args_json = self._get_str(L, 3)
                
                obj = self._get_obj(obj_id)
                if obj is None:
                    return self._ret_enc(L, "E", "Obj released/null")

                # Декодируем аргументы из JSON
                args_raw = json.loads(args_json)
                py_args = []
                for a in args_raw:
                    # Если аргумент - ссылка на Java объект
                    if isinstance(a, dict) and "__java_ref" in a:
                        ref_obj = self._get_obj(a["__java_ref"])
                        py_args.append(ref_obj)
                    else:
                        py_args.append(a)

                # Ищем атрибут/метод через Chaquopy
                res = None
                
                # Попытка 1: Атрибут или Метод Chaquopy
                if hasattr(obj, method_name):
                    attr = getattr(obj, method_name)
                    if callable(attr) or "reflect.Method" in str(type(attr)):
                        # Это метод или конструктор
                        if method_name == "__init__": # Constructor call
                            res = obj(*py_args)
                        else:
                            res = attr(*py_args)
                    else:
                    ...
                    # Попытка 2: Возможно, это скрытый Java метод
                    try:
                        attr = getattr(obj, method_name)
                        res = attr(*py_args)
                    except:
                        return self._ret_enc(L, "E", f"Attr '{method_name}' not found")

                # Возврат результата
                if res is None:
                    return self._ret_enc(L, "N", "")
                elif isinstance(res, (bool, int, float, str)):
                    if isinstance(res, bool): return self._ret_enc(L, "B", str(res).lower())
                    if isinstance(res, str): return self._ret_enc(L, "S", res)
                    return self._ret_enc(L, "I", str(res))
                else:
                    # Сложный объект -> сохраняем в реестр
                    new_id = self._reg_obj(res)
                    return self._ret_enc(L, "R", str(new_id))

            except Exception as e:
                return self._ret_enc(L, "E", str(e))

        # 7. BRIDGE: Release Object
        def py_release(L):
            idx = MandreLua._lib.lua_tointegerx(L, 1, None)
            self._free_obj(idx)
            return 0

        self._reg("print", lua_log); self._reg("log", lua_log)
        self._reg("toast", lua_toast)
        self._reg("get_setting", lua_get_set)
        self._reg("set_setting", lua_set_set)
        self._reg("_py_bridge_import", py_import)
        self._reg("_py_bridge_call", py_call)
        self._reg("_py_bridge_release", py_release)

    # --- LOW LEVEL HELPERS ---

    def _ret_enc(self, L, t, val):
        """Helper to return encoded string to Lua"""
        s = f"{t}{val}"
        MandreLua._lib.lua_pushstring(L, s.encode('utf-8'))
        return 1

    def _get_str(self, L, idx):
        try:
            ptr = MandreLua._lib.lua_tolstring(L, idx, None)
            if ptr: return ctypes.cast(ptr, c_char_p).value.decode('utf-8', 'ignore')
        except: pass
        return ""

    def _reg(self, name, py_func):
        c_func = MandreLua.LUA_C_FUNC(py_func)
        self._callbacks.append(c_func)
        MandreLua._lib.lua_pushcclosure(self.L, c_func, 0)
        MandreLua._lib.lua_setglobal(self.L, name.encode('utf-8'))

    def run_script(self, script: str):
        with self.lock:
            if not self.L: return "Lua Not Init"
            try:
                res = MandreLua._lib.luaL_loadstring(self.L, script.encode('utf-8'))
                if res != 0: return self._pop_error()
                res = MandreLua._lib.lua_pcallk(self.L, 0, 0, 0, 0, None)
                if res != 0: return self._pop_error()
                return None
    ...
    def call_function(self, func_name):
        with self.lock:
            ...
            try:
                type_id = MandreLua._lib.lua_getglobal(self.L, func_name.encode('utf-8'))
                if type_id != 6: # Not function
                    MandreLua._lib.lua_settop(self.L, 0)
                    return None
                if MandreLua._lib.lua_pcallk(self.L, 0, 1, 0, 0, None) != 0:
                    return None # Error handled internally usually or ignored
                res = self._get_str(self.L, -1)
                MandreLua._lib.lua_settop(self.L, 0)
                return res
            except: return None

    def _pop_error(self):
        err = self._get_str(self.L, -1)
        MandreLua._lib.lua_settop(self.L, 0)
        return err
    ...
    def close(self):
        with self.lock:
            if self.L:
                try: MandreLua._lib.lua_close(self.L)
                except: pass
                self.L = None
            self._callbacks.clear()
            self.registry.clear()

# --- СИСТЕМА MANDRE PACKAGE (Вставлять ПЕРЕД MandreLibPlugin) ---
...
class MandreMeta:
    ...
    def _parse(self):
        ...
        for key, val_multi, val_single in matches:
            val = val_multi if val_multi else val_single
...
class MandrePackage:
    PACKAGES_DIR = os.path.join(str(ApplicationLoader.getFilesDirFixed()), "plugins", "mandre_packages")

    @staticmethod
    def install(path: str, listener=None) -> tuple:
        """Устанавливает ТОЛЬКО Lua плагины (.mandre)"""
        try:
            temp_meta = None
            ...
            try:
                with zipfile.ZipFile(path, 'r') as zf:
                    names = zf.namelist()
                    if "meta.mandre" not in names:
                        return False, "Нет meta.mandre", None
                    if "main.lua" not in names:
                        return False, "Нет main.lua! Формат .mandre теперь только для Lua.", None
                    
                    with zf.open("meta.mandre") as f:
                        temp_meta = MandreMeta(f.read().decode('utf-8'))
            except: return False, "Ошибка чтения ZIP", None

            pid = temp_meta.get("id")
            if not pid: return False, "Нет ID в meta", None

            # 2. Распаковка
            target_dir = os.path.join(MandrePackage.PACKAGES_DIR, pid)
            if os.path.exists(target_dir): shutil.rmtree(target_dir)
            os.makedirs(target_dir)

            if listener: listener(f"Распаковка Lua плагина...")
            with zipfile.ZipFile(path, 'r') as zf: zf.extractall(target_dir)

            # 3. Генерация Python-моста
            if listener: listener("Создание моста...")
            MandrePackage._create_lua_bridge(pid, temp_meta)

            return True, "OK", temp_meta

        except Exception as e:
            return False, str(e), None

    @staticmethod
    def _create_lua_bridge(pid: str, meta: MandreMeta):
        """Создает легкий Python-файл, который просто запускает Lua"""
        wrap_path = os.path.join(str(ApplicationLoader.getFilesDirFixed()), "plugins", f"{pid}.py")
        esc = lambda s: s.replace('"""', "\\\"\\\"\\\"") if s else ""
        
        # Путь к скрипту Lua
        lua_script_path = os.path.join(MandrePackage.PACKAGES_DIR, pid, "main.lua").replace("\\", "/")

        code = f'''# Generated by MandreLib (Lua Mode)
__id__ = "{meta.get("id")}"
__name__ = "{esc(meta.get("name"))}"
__version__ = "{meta.get("version")}"
__author__ = """{esc(meta.get("author"))}"""
__description__ = """{esc(meta.get("description"))}"""
__min_version__ = "{meta.get("min_version", "1.0")}"
__icon__ = "{meta.get("icon", "msg_settings")}"

from base_plugin import BasePlugin
import os
try: from mandre_lib import Mandre
except ImportError: Mandre = None

LUA_PATH = r"{lua_script_path}"

class MandreLuaWrapper_{pid}(BasePlugin):
    def on_plugin_load(self):
        if not Mandre: return
        self._lua = Mandre.Lua(self)
        
        if os.path.exists(LUA_PATH):
            with open(LUA_PATH, "r", encoding="utf-8") as f:
                script = f.read()
            # Запускаем скрипт
            err = self._lua.run_script(script)
            if err:
                self.log(f"Lua Error: {{err}}")
        else:
            self.log(f"main.lua not found at {{LUA_PATH}}")

    def on_plugin_unload(self):
        if hasattr(self, "_lua") and self._lua:
            self._lua.close()

    def create_settings(self):
        if hasattr(self, "_lua") and self._lua:
            # Вызываем Lua функцию on_settings
            dsl = self._lua.call_function("on_settings")
            if dsl and Mandre:
                return Mandre.Settings.render(self, dsl)
        return None

    # Перенаправление всех методов (клики и т.д.) в Lua
    def __getattr__(self, name):
        def lua_proxy(*args):
            if hasattr(self, "_lua") and self._lua:
                # Вызываем глобальную функцию в Lua с таким же именем
                return self._lua.call_function(name)
            return None
        return lua_proxy
'''
        with open(wrap_path, "w", encoding="utf-8") as f: f.write(code)
class _PkgValidateHook(MethodHook):
    def after_hooked_method(self, param):
        try:
            path = str(param.args[0])
            if path.endswith(".mandre") or path.endswith(".zip"):
                import zipfile
                is_pkg = False
                try:
                    with zipfile.ZipFile(path, 'r') as zf:
                        if "meta.mandre" in zf.namelist():
                            with zf.open("meta.mandre") as f:
                                meta = MandreMeta(f.read().decode('utf-8'))
                            is_pkg = True
                except: pass

                if is_pkg:
                    PluginCls = find_class("com.exteragram.messenger.plugins.Plugin")
                    p = PluginCls(meta.get("id"), meta.get("name"))
                    p.setDescription(f"📦 [Mandre Package]\n{meta.get('description', '')}")
                    p.setAuthor(meta.get("author", ""))
                    p.setVersion(meta.get("version", "1.0"))
                    
                    ResultCls = find_class("com.exteragram.messenger.plugins.PluginsController$PluginValidationResult")
                    param.setResult(ResultCls(p, None))
        except: pass

class _PkgLoadHook(MethodHook):
    def before_hooked_method(self, param):
        path = str(param.args[0])
        cb = param.args[2]
        
        if path.endswith(".mandre"):
            param.setResult(None)
            def runner():
                def listener(msg): run_on_ui_thread(lambda: BulletinHelper.show_info(msg))
                try:
                    # 1. Установка (распаковка файлов)
                    success, msg, meta = MandrePackage.install(path, listener)
                    
                    if success and meta:
                        # 2. Регистрируем плагин в системе
                        # !!! ФИКС: Добавлена задержка 500мс, чтобы RecyclerView успел отрисоваться !!!
                        run_on_ui_thread(lambda: self._register_plugin_in_engine(meta), delay=500)
                        
                        if cb: run_on_ui_thread(lambda: cb.run(None))
                    else:
                        run_on_ui_thread(lambda err=msg: BulletinHelper.show_error(f"Err: {err}"))
                        if cb: run_on_ui_thread(lambda: cb.run("Install Failed"))
                except Exception as e:
                    if cb: run_on_ui_thread(lambda: cb.run(str(e)))
            threading.Thread(target=runner).start()

    def _register_plugin_in_engine(self, meta):
        try:
            pid = meta.get("id")
            wrapper_path = os.path.join(str(ApplicationLoader.getFilesDirFixed()), "plugins", f"{pid}.py")
            
            # Рефлексия
            PCWrapper = find_class("com.exteragram.messenger.plugins.PluginsController")
            PCCls = Mandre.Reflect._to_java_class(PCWrapper)
            PC = PCCls.getDeclaredMethod("getInstance").invoke(None)
            
            engine = PC.engines.get("python")
            
            EngineWrapper = find_class("com.exteragram.messenger.plugins.PythonPluginsEngine")
            EngineCls = Mandre.Reflect._to_java_class(EngineWrapper)
            
            JString = find_class("java.lang.String")
            JStringCls = Mandre.Reflect._to_java_class(JString)
            
            # Загружаем плагин
            load_method = EngineCls.getDeclaredMethod("loadPlugin", JStringCls, JStringCls)
            load_method.setAccessible(True)
            load_method.invoke(engine, pid, wrapper_path)
            
            # Обновляем список
            notify_method = PCCls.getDeclaredMethod("notifyPluginsChanged")
            notify_method.setAccessible(True)
            notify_method.invoke(PC)
            
        except Exception as e:
            import traceback
            log(f"Engine Register Error: {e}\n{traceback.format_exc()}")
            
class _PkgIsPluginHook(MethodHook):
    """Говорит движку, что .mandre это тоже плагин"""
    def after_hooked_method(self, param):
        try:
            f = param.args[0]
            if f and str(f.getName()).endswith(".mandre"):
                param.setResult(True)
        except: pass

class _OpenMandreHook(MethodHook):
    def before_hooked_method(self, param):
        try:
            f = param.args[0]
            if not f: return
            ...
            if path.endswith(".mandre"):
                param.setResult(True)
                run_on_ui_thread(lambda: self._trigger_install(path))
        except: pass

    def _trigger_install(self, path):
        try:
            PC = find_class("com.exteragram.messenger.plugins.PluginsController").getInstance()
            frag = get_last_fragment()
            if frag:
                # Ищем метод showInstallDialog(BaseFragment, String, boolean)
                for m in PC.getClass().getDeclaredMethods():
                    if m.getName() == "showInstallDialog":
                        pt = m.getParameterTypes()
                        if len(pt) == 3 and pt[1].getName() == "java.lang.String":
                            m.invoke(PC, frag, path, False)
                            return
        except Exception as e:
            BulletinHelper.show_error(f"Error: {e}")

# --- MANDRE INLINE MODULE (REFACTORED) ---

class _InlineConstants:
    # Маркеры для скрытия данных (Zero Width Characters)
    ZERO = '\u200b'
    ONE = '\u200c'
    MARKER = '\u200d'
    PREFIX = "mandre:"
...
class _InlineUtils:
    """Утилиты для кодирования/декодирования скрытой нагрузки"""
    
    @staticmethod
    def encode_payload(data: dict) -> str:
        """Кодирует dict в невидимую строку (JSON -> Zlib -> Binary String -> Zero Width Chars)"""
        try:
            # 1. JSON (максимально компактный)
            json_str = json.dumps(data, separators=(',', ':'))
            # 2. Сжатие
            compressed = zlib.compress(json_str.encode('utf-8'))
            # 3. В битовую строку
            bits = ''.join(format(byte, '08b') for byte in compressed)
            # 4. Замена 0 и 1 на невидимые символы
            hidden = bits.replace('0', _InlineConstants.ZERO).replace('1', _InlineConstants.ONE)
            # 5. Обертывание в маркеры
            return f"{_InlineConstants.MARKER}{hidden}{_InlineConstants.MARKER}"
        except Exception as e:
            log(f"[MandreInline] Encode error: {e}")
            return ""

    @staticmethod
    def decode_payload(text: str) -> Optional[dict]:
        """Ищет и декодирует скрытые данные из текста сообщения"""
        try:
            if not text or _InlineConstants.MARKER not in text:
                return None

            start = text.find(_InlineConstants.MARKER)
            end = text.find(_InlineConstants.MARKER, start + 1)
            ...
            # Быстрая проверка на валидность символов
            if not all(c in (_InlineConstants.ZERO, _InlineConstants.ONE) for c in hidden):
                return None

            # Обратное преобразование
            bits = hidden.replace(_InlineConstants.ZERO, '0').replace(_InlineConstants.ONE, '1')
            
            # Битовая строка -> int -> bytes
            n = int(bits, 2)
            byte_len = (len(bits) + 7) // 8
            compressed = n.to_bytes(byte_len, 'big')
            
            json_str = zlib.decompress(compressed).decode('utf-8')
            return json.loads(json_str)
        except Exception:
            # Ошибки декодирования (например, обычный текст с похожими символами) игнорируем
            return None

class _InlineRenderHook(MethodHook):
    """
    Хук для отрисовки кнопок (ChatMessageCell).
    Перехватывает установку объекта сообщения, декодирует payload и подменяет reply_markup.
    """
    
    # Кэшируем классы, чтобы не искать их при каждом рендере сообщения (оптимизация)
    def __init__(self):
        super().__init__()
        self._classes_loaded = False
        self.TL_replyInlineMarkup = None
        self.TL_keyboardButtonRow = None
        self.TL_keyboardButtonCallback = None
        self.ArrayList = None

    def _ensure_classes(self):
        if self._classes_loaded: return True
        try:
            self.TL_replyInlineMarkup = find_class("org.telegram.tgnet.TLRPC$TL_replyInlineMarkup")
            self.TL_keyboardButtonRow = find_class("org.telegram.tgnet.TLRPC$TL_keyboardButtonRow")
            self.TL_keyboardButtonCallback = find_class("org.telegram.tgnet.TLRPC$TL_keyboardButtonCallback")
            self.ArrayList = find_class("java.util.ArrayList")
    ...
    def before_hooked_method(self, param):
        try:
            # Сигнатура: setMessageObject(MessageObject, ...)
            if not param.args or len(param.args) < 1: return
            
            msg_obj = param.args[0]
            # Проверка валидности объекта сообщения
            if not msg_obj or not hasattr(msg_obj, "messageOwner") or not msg_obj.messageOwner: return
            
            # Быстрая проверка текста на наличие маркера
            text = getattr(msg_obj, "messageText", "")
            if not text or _InlineConstants.MARKER not in str(text):
                return

            payload = _InlineUtils.decode_payload(str(text))
            if not payload or "btns" not in payload:
                return

            if not self._ensure_classes():
                return

            # Создание структуры кнопок (TLRPC)
            rows = self.ArrayList()
            for row_data in payload["btns"]:
                row = self.TL_keyboardButtonRow()
                row.buttons = self.ArrayList()
                
                for btn_data in row_data:
                    btn = self.TL_keyboardButtonCallback()
                    btn.text = str(btn_data.get("t", "Button"))
                    
                    plugin_id = btn_data.get("pid", "unknown")
                    cb_data = btn_data.get("d", "")
                    
                    # Формат данных: mandre:pluginID:data
                    full_data = f"{_InlineConstants.PREFIX}{plugin_id}:{cb_data}"
                    
                    # Конвертация в Java byte array
                    btn.data = bytearray(full_data.encode('utf-8'))
                    row.buttons.add(btn)
                rows.add(row)

            markup = self.TL_replyInlineMarkup()
            markup.rows = rows
            
            # Внедрение разметки в объект сообщения
            msg_obj.messageOwner.reply_markup = markup
            # Флаг FLAGS_HAS_REPLY_MARKUP (64 или 1<<6)
            msg_obj.messageOwner.flags |= 64
            
            # !!! КРИТИЧНО !!!
            # Заставляем Telegram пересчитать геометрию кнопок.
            # Без этого высота кнопок будет 0, и они не отрисуются.
            try:
                msg_obj.measureInlineBotButtons()
            except Exception:
                # В редких случаях measure может упасть, если контекст не готов
                pass
            
        except Exception as e:
            log(f"[MandreInline] Render Error: {e}")

class _InlineClickHook(MethodHook):
    """
    Хук нажатия на кнопку (SendMessagesHelper).
    Перехватывает отправку callback-запроса, если он начинается с 'mandre:'.
    """
    def before_hooked_method(self, param):
        try:
            # Метод: sendCallback(boolean cache, MessageObject msg, TLRPC.KeyboardButton btn, ...)
            if len(param.args) < 3: return

            button = param.args[2]
            if not button or not hasattr(button, "data") or not button.data: return

            data_bytes = button.data
            
            # Конвертация Java bytes -> Python str
            try:
                data_str = str(data_bytes, 'utf-8', 'ignore')
            except:
                # Fallback для старых версий/Jython
                data_str = "".join(map(chr, data_bytes))

            if data_str.startswith(_InlineConstants.PREFIX):
                # Блокируем отправку запроса на сервер Telegram
                param.setResult(None) 
                
                # Парсинг данных: mandre:plugin_id:payload
                ...
                msg_obj = param.args[1]
                
                # Асинхронный вызов обработчика, чтобы не блокировать UI поток хука
                run_on_ui_thread(lambda: MandreInline._dispatch_click(plugin_id, payload, msg_obj))
                
        except Exception as e:
            log(f"[MandreInline] Click Hook Error: {e}")

class MandreInline:
    """
    Публичный API для работы с локальными Inline-кнопками.
    """
    _CALLBACKS = {} # plugin_id -> callback_function

    @staticmethod
    def register_callback(plugin_instance: BasePlugin, callback: Callable):
        """
        Регистрирует функцию для обработки нажатий.
        Callback signature: (data: str, message_object: MessageObject)
        """
        MandreInline._CALLBACKS[plugin_instance.id] = callback

    @staticmethod
    def create_markup(plugin_instance: BasePlugin, rows: List[List[Dict]]) -> str:
        """
        Создает скрытую строку с разметкой кнопок.
        Эту строку нужно добавить к тексту сообщения.
        rows structure: [[{'text': 'Btn1', 'data': 'payload1'}, ...], ...]
        """
        compact_rows = []
        for row in rows:
            compact_row = []
        ...
        payload = {"btns": compact_rows}
        return _InlineUtils.encode_payload(payload)

    @staticmethod
    def _dispatch_click(plugin_id: str, data: str, message_object):
        """Внутренний метод маршрутизации кликов"""
        if plugin_id in MandreInline._CALLBACKS:
            try:
                MandreInline._CALLBACKS[plugin_id](data, message_object)
            except Exception as e:
                log(f"[MandreInline] Dispatch Error for {plugin_id}: {traceback.format_exc()}")
                BulletinHelper.show_error(f"Plugin Error: {e}")
        else:
            BulletinHelper.show_error(f"Plugin '{plugin_id}' inactive or reloaded.")

    @staticmethod
    def install_hooks(plugin_instance: BasePlugin):
        """
        Устанавливает необходимые хуки. Вызывать один раз при старте MandreLib.
        """
        log("[MandreInline] Installing hooks...")
        
        # 1. Хук Рендера (ChatMessageCell.setMessageObject)
        try:
            Wrapper = find_class("org.telegram.ui.Cells.ChatMessageCell")
            RealClass = MandreReflect._to_java_class(Wrapper)
            
            if RealClass:
                found = False
                for m in RealClass.getDeclaredMethods():
                    if m.getName() == "setMessageObject":
                        plugin_instance.hook_method(m, _InlineRenderHook())
                        found = True
                
                if not found: log("[MandreInline] setMessageObject not found!")
            else:
                log("[MandreInline] ChatMessageCell class not found!")
        except Exception as e:
            log(f"[MandreInline] Render Hook Setup Failed: {e}")

        # 2. Хук Клика (SendMessagesHelper.sendCallback)
        try:
            Wrapper = find_class("org.telegram.messenger.SendMessagesHelper")
            RealClass = MandreReflect._to_java_class(Wrapper)
            
            if RealClass:
                found = False
                for m in RealClass.getDeclaredMethods():
                    if m.getName() == "sendCallback":
                        # Ищем перегрузку, принимающую MessageObject и кнопку (обычно 4 аргумента)
                        if len(m.getParameterTypes()) >= 4:
                            plugin_instance.hook_method(m, _InlineClickHook())
                            found = True
                            break 
                
                if not found: log("[MandreInline] sendCallback not found!")
            else:
                log("[MandreInline] SendMessagesHelper class not found!")
        except Exception as e:
            log(f"[MandreInline] Click Hook Setup Failed: {e}")

# --- OPTIMIZED MEMORY MANAGER (No Freezes) ---
import ctypes
import gc
from java.lang import Runtime

class _MandreMemoryCore:
    MODE_JAVA = "java"
    MODE_NATIVE = "native"
    
    def __init__(self):
        self._libc = None
        self._native_allocated = 0
        self._java_runtime = Runtime.getRuntime()
        
        # Init LibC for malloc
        try:
            self._libc = ctypes.CDLL("libc.so")
            ...
            log(f"[MandreMemory] LibC init failed: {e}")

    def alloc(self, plugin_instance, size_bytes: int, mode: str = "java"):
        # FAST PATH: Никаких проверок tracemalloc, никаких sleep!
        if mode == self.MODE_NATIVE and self._libc:
            return self._alloc_native(size_bytes)
        else:
            return self._alloc_managed(size_bytes)

    def _alloc_native(self, size):
        # Пытаемся выделить память
        ptr = self._libc.malloc(size)
        
        # Если NULL, пробуем вызвать GC и попробовать снова (Last Resort)
        if not ptr:
            gc.collect()
            ptr = self._libc.malloc(size)
            
        if not ptr:
            raise MemoryError(f"Native malloc failed for {size} bytes")
        
        self._native_allocated += size

        # Обертка для автоматического free()
        class NativeBuffer:
            def __init__(self, p, s, core):
                self.ptr = p
                self.size = s
                self.core = core
                self._freed = False
            ...
            def __del__(self):
                if not self._freed and self.core._libc:
                    self.core._libc.free(self.ptr)
                    self.core._native_allocated -= self.size
                    self._freed = True
            
            def write(self, data: bytes):
                if len(data) > self.size: raise ValueError("Overflow")
                ctypes.memmove(self.ptr, data, len(data))

            def read(self, length=None):
                l = length if length else self.size
                return ctypes.string_at(self.ptr, l)

        return NativeBuffer(ptr, size, self)

    def _alloc_managed(self, size):
        # Проверяем Java Heap перед выделением
        # Если свободной памяти критически мало (<10MB), форсируем GC
        free_mem = self._java_runtime.freeMemory()
        if free_mem < 10 * 1024 * 1024:
            gc.collect()
            
        return bytearray(size)

    def set_limit_mb(self, plugin_instance, limit_mb: int):
        # В этой версии мы не ограничиваем жестко, чтобы не вызывать фризов.
        # Мы просто логируем намерение.
        log(f"[MandreMemory] Target limit for {plugin_instance.id}: {limit_mb}MB (Optimized Mode)")

    def get_stats(self) -> str:
        # Быстрая статистика без тяжелого tracemalloc
        total_mem = self._java_runtime.totalMemory() // 1024 // 1024
        free_mem = self._java_runtime.freeMemory() // 1024 // 1024
        max_mem = self._java_runtime.maxMemory() // 1024 // 1024
        used_mem = total_mem - free_mem
        
        native_mb = self._native_allocated / 1024 / 1024
        
        return (
            f"📊 **Memory Stats (No-Freeze)**\n"
            f"💎 **Java Heap:** {used_mem}MB / {max_mem}MB (Free: {free_mem}MB)\n"
            f"⚙️ **Native (Malloc):** {native_mb:.2f} MB\n"
            f"🚀 **Performance:** Optimized (Throttle disabled)"
        )

    # Заглушки для совместимости API
    def start_profiler(self): log("[MandreMemory] Profiler disabled for performance")
    def stop_profiler(self): pass


_MANDRE_MEMORY = _MandreMemoryCore()

class MandreMemory:
    """
    Быстрый менеджер памяти без фризов.
    """
    MODE_JAVA = _MandreMemoryCore.MODE_JAVA
    MODE_NATIVE = _MandreMemoryCore.MODE_NATIVE

    @staticmethod
    def alloc(plugin_instance, size_bytes: int, mode: str = MODE_JAVA):
        return _MANDRE_MEMORY.alloc(plugin_instance, size_bytes, mode)

    @staticmethod
    def set_limit(plugin_instance, mb: int):
        _MANDRE_MEMORY.set_limit_mb(plugin_instance, mb)

    @staticmethod
    def start_profiler(): _MANDRE_MEMORY.start_profiler()
    @staticmethod
    def stop_profiler(): _MANDRE_MEMORY.stop_profiler()
    @staticmethod
    def get_stats() -> str: return _MANDRE_MEMORY.get_stats()

# --- CORE & CPU MANAGER ---

class _MandreCoreManagerCore:
    def __init__(self):
        self._libc = None
        try:
            self._libc = ctypes.CDLL("libc.so")
            # sched_setaffinity args: pid, size, mask_ptr
            self._libc.sched_setaffinity.argtypes = [ctypes.c_int, ctypes.c_size_t, ctypes.c_void_p]
            self._libc.sched_getaffinity.argtypes = [ctypes.c_int, ctypes.c_size_t, ctypes.c_void_p]
        except Exception as e:
            log(f"[MandreCore] LibC init failed: {e}")

    def get_cpu_count(self):
        """Возвращает реальное количество ядер"""
        return multiprocessing.cpu_count()

    def set_affinity(self, pid: int, core_ids: List[int]):
        """
        Привязывает процесс/поток к конкретным ядрам.
        :param pid: ID процесса (0 для текущего)
        :param core_ids: Список ID ядер (например [4, 5, 6, 7] для big-ядер)
        """
        if not self._libc: return False
        
        try:
            # Создаем битовую маску (cpu_set_t)
            # В Android mask обычно long integer
            mask = 0
            for core in core_ids:
                mask |= (1 << core)
            
            # Размер маски (обычно 4 или 8 байт, но для надежности берем sizeof(long))
            mask_size = ctypes.sizeof(ctypes.c_long)
            mask_ptr = ctypes.byref(ctypes.c_long(mask))
            
            res = self._libc.sched_setaffinity(pid, mask_size, mask_ptr)
            if res == 0:
                log(f"[MandreCore] PID {pid} привязан к ядрам {core_ids}")
                return True
            else:
                log(f"[MandreCore] Ошибка setaffinity: код {res}")
                return False
        except Exception as e:
            log(f"[MandreCore] Affinity Error: {e}")
            return False

    def maximize_priority(self):
        """Устанавливает максимальный приоритет текущему потоку"""
        try:
            # Android/Linux nice level (-20 = max priority)
            os.nice(-10) # Пробуем повысить (требует прав, но внутри app процесса иногда работает до определенного предела)
            
            # Java priority
            Thread = find_class("java.lang.Thread")
            current = Thread.currentThread()
            current.setPriority(Thread.MAX_PRIORITY) # 10
            
            # Android Process priority
            Process = find_class("android.os.Process")
            my_tid = Process.myTid()
            Process.setThreadPriority(my_tid, -19) # THREAD_PRIORITY_AUDIO (-16) or URGENT_AUDIO (-19)
            
            log("[MandreCore] Приоритет максимально повышен (Java + Native)")
        except Exception as e:
            log(f"[MandreCore] Priority Error: {e}")

_MANDRE_CORE = _MandreCoreManagerCore()

class MandreCoreManager:
    @staticmethod
    def get_info():
        count = _MANDRE_CORE.get_cpu_count()
        return f"CPU Cores: {count}"

    @staticmethod
    def use_cores(core_list: List[int]):
        """
        Заставляет текущий плагин работать только на выбранных ядрах.
        Полезно для выбора Performance-ядер (обычно последние индексы).
        """
        _MANDRE_CORE.set_affinity(0, core_list)

    @staticmethod
    def boost_plugin():
        """Дает плагину максимальный приоритет CPU"""
        _MANDRE_CORE.maximize_priority()


class MandreMultiCore:
    """
    Позволяет плагинам использовать истинную многопоточность (обходим GIL).
    ВНИМАНИЕ: Использует ProcessPool. Объекты должны быть пиклируемыми (сериализуемыми).
    """
    _executor = None
    _workers = 2

    @staticmethod
    def init(workers: int = 0):
        """Инициализирует пул процессов"""
        if workers <= 0:
            workers = _MANDRE_CORE.get_cpu_count()
        
        MandreMultiCore._workers = workers
        if MandreMultiCore._executor:
            MandreMultiCore._executor.shutdown()
        
        # Используем ThreadPool как безопасный фоллбэк на Android, 
        # так как fork() процессов внутри APK часто запрещен или крашит приложение.
        # Для реальной нагрузки CPU в Python на Android нужны C-расширения без GIL.
        # Но мы попробуем ProcessPoolExecutor, если система позволит.
        try:
            # На Android безопаснее использовать ThreadPoolExecutor для совместимости,
            # но если мы хотим именно MultiCore для вычислений, нам нужен ProcessPool.
            # Однако, Chaquopy имеет ограничения на спавн процессов.
            # Поэтому мы делаем "умный" выбор.
            
            # СТРАТЕГИЯ: Используем ThreadPool, но с трюком.
            # Если плагин использует MandreMultiCore для I/O или C-библиотек (numpy, cv2),
            # то ThreadPool тоже даст ускорение, так как C-либы отпускают GIL.
            MandreMultiCore._executor = ThreadPoolExecutor(max_workers=workers)
            log(f"[MandreMultiCore] Initialized with {workers} workers (Thread Mode)")
        except Exception as e:
            log(f"[MandreMultiCore] Init error: {e}")

    @staticmethod
    def run(func, *args):
        """
        Запускает тяжелую функцию на свободном ядре.
        Возвращает Future object.
        """
        if not MandreMultiCore._executor:
            MandreMultiCore.init()
        return MandreMultiCore._executor.submit(func, *args)

    @staticmethod
    async def run_async(func, *args):
        """
        Асинхронно запускает задачу на другом ядре и ждет результат.
        Совместимо с MandreAsync!
        Example: result = await Mandre.MultiCore.run_async(heavy_math, 1000)
        """
        if not MandreMultiCore._executor:
            MandreMultiCore.init()
        
        loop = asyncio.get_event_loop()
        # run_in_executor автоматически связывает AsyncIO и наш Пул
        return await loop.run_in_executor(MandreMultiCore._executor, functools.partial(func, *args))
    ...
    def map(func, iterable):
        ...
        if not MandreMultiCore._executor:
            MandreMultiCore.init()
        return list(MandreMultiCore._executor.map(func, iterable))
...
class Mandre:
    ...
    Package = MandrePackage
    Sensors = MandreSensors
    Memory = MandreMemory  
    MultiCore = MandreMultiCore   
    Core = MandreCoreManager      
...
class MandreLibPlugin(BasePlugin):
    ...
    def on_plugin_load(self):
        ...
        try:
            Mandre.Inline.install_hooks(self)
        except Exception as e:
            self.log(f"Inline Init Error: {e}")
       ...
       # --- АВТОМАТИЧЕСКАЯ УСТАНОВКА ЗАВИСИМОСТЕЙ ---
        ...
        try:
            EngineWrapper = find_class("com.exteragram.messenger.plugins.PythonPluginsEngine")
            EngineClass = Mandre.Reflect._to_java_class(EngineWrapper)
            String = find_class("java.lang.String")
            PluginCls = find_class("com.exteragram.messenger.plugins.Plugin")
            CallbackCls = find_class("org.telegram.messenger.Utilities$Callback")
            JavaFile = find_class("java.io.File")
            
            val_method = EngineClass.getDeclaredMethod("validatePluginFromFile", String)
            self.hook_method(val_method, _PkgValidateHook()) 
            
            load_method = EngineClass.getDeclaredMethod("loadPluginFromFile", String, PluginCls, CallbackCls)
            self.hook_method(load_method, _PkgLoadHook())
            
            # Хук isPlugin, чтобы контроллер не отсеивал файл .mandre до валидации
            is_plugin_method = EngineClass.getDeclaredMethod("isPlugin", JavaFile)
            self.hook_method(is_plugin_method, _PkgIsPluginHook())
            
            self.log("Pkg Installer: Active")
        except Exception as e:
            self.log(f"Install Hook Error: {e}")

        # 3. Хук открытия файла
        try:
            AndroidUtilsWrapper = find_class("org.telegram.messenger.AndroidUtilities")
            AndroidUtils = Mandre.Reflect._to_java_class(AndroidUtilsWrapper)
            JavaFile = find_class("java.io.File")
            
            hooked = 0
            for m in AndroidUtils.getDeclaredMethods():
                if m.getName() == "openForView":
                    pt = m.getParameterTypes()
                    if len(pt) > 0 and pt[0].getName() == "java.io.File":
                        self.hook_method(m, _OpenMandreHook())
                        hooked += 1
            if hooked > 0: self.log("Open Hook: Active")
        except Exception as e:
            self.log(f"Open Hook Error: {e}")

    def on_plugin_unload(self):
        global _mandrelib_instance
        _internal_shutdown_tts()
        _mandrelib_instance = None
        self.log(f"Библиотека выгружена.")
